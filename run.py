...
def scrape():
    ...
    WebDriverWait(bot, TIMEOUT).until(
        EC.presence_of_element_located((
            By.XPATH, '//*[starts-with(@id, "mount_0")]/div/div/div/div/div/div/div/div/section/main/div/ul/li[2]/a'))).click()
    ...
    for _ in range(round(user_input // 10)):
        ...
        followers = bot.find_elements_by_xpath(
            '//*[starts-with(@id, "mount_0")]/div/div/div/div/div/div/div/div/section/main/div/div/div/div')
       
        # Getting url from href attribute
        for i in followers:
            # -----------
            # I'm not sure if this line of code gets the HTML tag that contains the username
            # It needs to be verified, please
            # Currently instagram restricts my access to followers after approx 10 attempts
            # -----------
            row = i.find_element(By.XPATH,'//div[2]/div/div/div/a')
            print(row)
            if row.get_attribute('href'):
                users.add(row.get_attribute('href').split("/")[3])
            else:
                continue
